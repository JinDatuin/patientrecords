...
public function public_search() {
    ...
    if (!empty($search_term)) {
        ...
        $data['get_data'] = $this->Addusers_model->search_patients($search_term);
    }

    // This must match the file name: application/views/public_patient_search.php
    $this->load->view('public_patient_search', $data);
}
...
// application/controllers/Admissioncontrol.php

public function patientdataview($id) {
    // Fetch the primary patient details
    $data['get_data'] = $this->Record_model->get_patient_by_id($id); 
    
    // Fetch the arrays for the tables (This is where your error was occurring)
    $data['get_findings_data'] = $this->Record_model->get_findings_by_patient($id);
    $data['get_admission_data'] = $this->Record_model->get_admissions_by_patient($id);

    if(empty($data['get_data'])) {
        show_404();
    }

    $this->load->view('admission/patientdataview', $data);
}
  ...
  $date = date("Y-m-d");
   ...
   'f_date' => $date
    ...
    if($this->Record_model->add_findings_data($pr_id, $data)){
      ...
      redirect('admissioncontrol/admit_form/'.$pr_id.'#findings', $data);
